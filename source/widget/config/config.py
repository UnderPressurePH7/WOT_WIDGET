...
from widget.config.config_param import g_configParams
...
class Config(object):
    ...
    def load_config(self):
        try:
            ...
            print_error("[Config] Error loading config: %s" % str(e))
    ...
    def save_config(self):
        try:
            ...
            print_error("[Config] Error saving config: %s" % str(e))
    ...
    def _get_safe_msa_value(self, param):
        try:
            ...
            print_error("[Config] Error getting msaValue for %s: %s" % (param.tokenName, str(e)))
    ...
    def _register_mod(self):
        try:
            ...
            print_error("[Config] Error registering mod template: %s" % str(e))
    ...
    def on_settings_changed(self, linkage, newSettings):
        ...
        try:
            ...
            print_error("[Config] Error updating settings from MSA: %s" % str(e))
    ...
    def _notify_config_changed(self):
        try:
            ...
            print_error("[Config] Error notifying config change: %s" % str(e))

    def sync_with_msa(self):
        try:
            print_debug("[Config] MSA sync called - using config file values")
        except Exception as e:
            print_error("[Config] Error in MSA sync: %s" % str(e))
